...
class valueMomentumClass:
    ...
    def obtener_composite_value_tickers(self, tickers, sector ='fin'):
       ...
       Y el factor QUALITY para una lista de tickers.

       VALUE   : P/E, P/B normalizados con z-score INVERSO (barato = mejor).
       QUALITY : ROE, ROA, margen y crecimiento con z-score DIRECTO (mayor = mejor).
                 Mide si la empresa es 'maravillosa', no solo barata.
       ValueQuality_z : blend 50/50 al estilo Buffett
       ...
       Referencias:
         ...
         - Frazzini, Kabiller & Pedersen (2018), "Buffett's Alpha", FAJ 74.

       Aplica filtros absolutos para evitar que acciones 'caras' distorsionen el ranking.
       Devuelve un DataFrame ordenado (mayor ValueQuality_z = más atractiva).
       """
       import pandas as pd
       import numpy as np
       import yfinance as yf
       ...
       for t in tickers:
           try:
               ...
               data.append({
                   ...
                   # --- QUALITY (Buffett / QMJ): mayor = mejor ---
                   "ROE": info.get("returnOnEquity", np.nan),            # rentabilidad sobre fondos propios
                   "ROA": info.get("returnOnAssets", np.nan),            # rentabilidad sobre activos
                   "ProfitMargin": info.get("profitMargins", np.nan),    # margen neto
                   "EarningsGrowth": info.get("earningsGrowth", np.nan), # crecimiento de beneficios
       ...
       # 7️⃣ QUALITY (factor calidad estilo Buffett / QMJ)
       #    OJO: z-score DIRECTO (sin invertir el signo) porque aquí MAYOR = MEJOR,
       #    al revés que en value, donde barato (z-score inverso) = mejor.
       quality_cols = ["ROE", "ROA", "ProfitMargin", "EarningsGrowth"]
       for col in quality_cols:
           df[col] = pd.to_numeric(df[col], errors="coerce")

       q_scores = df[quality_cols].apply(
           lambda x: (x - np.nanmean(x)) / np.nanstd(x)
       )
       # media por FILA (skipna por defecto: tolera métricas de calidad ausentes)
       df["Quality"] = q_scores.mean(axis=1)
       # z-score del Quality por COLUMNA, misma escala que Composite_z
       df["Quality_z"] = (df["Quality"] - df["Quality"].mean()) / df["Quality"].std()

       # 8️⃣ Blend VALUE + QUALITY al estilo Buffett (50/50)
       #    "wonderful company (Quality) at a fair price (Value)"
       df["ValueQuality_z"] = 0.5 * df["Composite_z"] + 0.5 * df["Quality_z"]

       # 9️⃣ Ranking final: ahora ordenamos por el blend Value+Quality
       df["Ranking"] = df["ValueQuality_z"].rank(ascending=False)
       ...
       return df[["Ticker", "Sector", "P/E", "P/B", "EV/EBITDA", "P/S",
                   ...
                   "Composite Value", "Composite_z",
                   "Quality", "Quality_z", "ValueQuality_z", "Ranking"]]
